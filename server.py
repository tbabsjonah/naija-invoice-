#!/usr/bin/env python3
"""
FlowLedger - Invoicing & bookkeeping tool for Nigerian businesses
...
def init_db():
    ...
    """)
    # Migration: add payment_method and payment_date if missing
    try:
        conn.execute("SELECT payment_method FROM invoices LIMIT 1")
    except sqlite3.OperationalError:
        conn.execute("ALTER TABLE invoices ADD COLUMN payment_method TEXT DEFAULT ''")
        conn.execute("ALTER TABLE invoices ADD COLUMN payment_date TEXT DEFAULT ''")
    conn.commit()
    conn.close()
...
def generate_invoice_html(invoice_id):
    ...
    if inv["status"] == "paid" and inv.get("payment_method"):
        ...
        <div style="margin-top:15px;padding:15px;background:#f0fdf4;border-radius:8px;border:1px solid #bbf7d0;">
            <p style="margin:0;font-size:13px;"><strong>Payment Method:</strong> {method_label}</p>
            {('<p style="margin:3px 0;font-size:13px;"><strong>Payment Date:</strong> ' + inv.get("payment_date", "") + '</p>') if inv.get("payment_date") else ""}
        </div>"""
        ...
        if path == "/" or path == "":
            ...
            rows = conn.execute("""
                SELECT c.id, c.name, c.company,
                    ...
                    COUNT(CASE WHEN i.type='invoice' THEN 1 END) as invoice_count,
                    COUNT(CASE WHEN i.type='invoice' AND i.status='paid' THEN 1 END) as paid_count,
                    COUNT(CASE WHEN i.type='invoice' AND i.status='overdue' THEN 1 END) as overdue_count
                FROM clients c
            ...
            totals = {
                "total_invoiced": sum(r["total_invoiced"] for r in data),
                "total_paid": sum(r["total_paid"] for r in data),
                "total_pending": sum(r["total_pending"] for r in data),
            }
            conn.close()
            self._send_json({"clients": data, "totals": totals})
        elif path == "/api/reports/pnl":
            conn = get_db()
            revenue = conn.execute("SELECT COALESCE(SUM(subtotal),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            vat_collected = conn.execute("SELECT COALESCE(SUM(vat_amount),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            discounts = conn.execute("SELECT COALESCE(SUM(discount_amount),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            total_received = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            ...
            monthly = [dict(r) for r in conn.execute("""
                SELECT strftime('%Y-%m', issue_date) as month,
                    ...
                    COUNT(*) as count
                FROM invoices WHERE type='invoice' AND status='paid'
                GROUP BY month ORDER BY month DESC LIMIT 12
            """).fetchall()]
            conn.close()
            self._send_json({
                "revenue": revenue, "vat_collected": vat_collected,
                "discounts": discounts, "total_received": total_received,
                "net_income": revenue - discounts,
                "monthly": monthly,
            })
        elif path == "/api/reports/balance":
            conn = get_db()
            total_paid = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            total_receivable = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='invoice' AND status IN ('sent','overdue')").fetchone()["s"]
            total_draft = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='invoice' AND status='draft'").fetchone()["s"]
            vat_liability = conn.execute("SELECT COALESCE(SUM(vat_amount),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            total_quotes_pending = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='quote' AND status IN ('draft','sent')").fetchone()["s"]
            conn.close()
            self._send_json({
                "assets": {
                    "cash_received": total_paid,
                    "accounts_receivable": total_receivable,
                    "draft_invoices": total_draft,
                    "total": total_paid + total_receivable,
                },
                "liabilities": {
                    "vat_payable": vat_liability,
                    "total": vat_liability,
                },
                "equity": {
                    "retained_earnings": total_paid - vat_liability,
                    "total": total_paid + total_receivable - vat_liability,
                },
                "pending_quotes": total_quotes_pending,
            })
            ...
            # Monthly cash flow
            monthly = [dict(r) for r in conn.execute("""
                SELECT strftime('%Y-%m', COALESCE(payment_date, issue_date)) as month,
                    COALESCE(SUM(total),0) as inflow, COUNT(*) as count
                FROM invoices WHERE type='invoice' AND status='paid'
                GROUP BY month ORDER BY month DESC LIMIT 12
            """).fetchall()]
            total_inflow = conn.execute("SELECT COALESCE(SUM(total),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            conn.close()
            self._send_json({
            ...
            vat_paid = conn.execute("SELECT COALESCE(SUM(vat_amount),0) as s FROM invoices WHERE type='invoice' AND status='paid'").fetchone()["s"]
            vat_outstanding = conn.execute("SELECT COALESCE(SUM(vat_amount),0) as s FROM invoices WHERE type='invoice' AND status IN ('sent','overdue')").fetchone()["s"]
            vat_total = conn.execute("SELECT COALESCE(SUM(vat_amount),0) as s FROM invoices WHERE type='invoice'").fetchone()["s"]

            # Per-invoice tax detail
            invoices = [dict(r) for r in conn.execute("""
                SELECT i.invoice_number, i.issue_date, i.status, i.subtotal, i.vat_amount, i.total,
                    i.discount_amount, c.name as client_name
                FROM invoices i JOIN clients c ON i.client_id = c.id
                WHERE i.type='invoice' AND i.vat_amount > 0
                ORDER BY i.issue_date DESC
            """).fetchall()]

            # Monthly VAT breakdown
            monthly_vat = [dict(r) for r in conn.execute("""
                SELECT strftime('%Y-%m', issue_date) as month,
                    COALESCE(SUM(CASE WHEN status='paid' THEN vat_amount ELSE 0 END),0) as vat_collected,
                    COALESCE(SUM(CASE WHEN status IN ('sent','overdue') THEN vat_amount ELSE 0 END),0) as vat_pending,
                    COALESCE(SUM(subtotal - discount_amount),0) as taxable_income
                FROM invoices WHERE type='invoice'
                GROUP BY month ORDER BY month DESC LIMIT 12
            """).fetchall()]
            conn.close()
            self._send_json({
                "vat_rate": VAT_RATE,
                "vat_collected": vat_paid,
    ...
    def do_PUT(self):
        ...
        if path.startswith("/api/clients/"):
            ...
            if data.get("payment_date"):
                sets.append("payment_date = ?")
                vals.append(data["payment_date"])
